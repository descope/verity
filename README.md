...
```text
  copa-config.yaml
   ...
   │  Sign   │     CycloneDX SBOM attached as an in-toto attestation
...
Both are cosign-signed (keyless OIDC via Sigstore) with an SBOM attestation
attached via [`actions/attest`](https://github.com/actions/attest) and a
corresponding entry in the Rekor transparency log. Copa-patched images ship a
CycloneDX SBOM generated by Syft; Integer images ship an SPDX SBOM produced by
apko during the build. FIPS variants are available for Go-backed images (golang, caddy,
docker-cli, etcd, helm, terraform, cosign, crane, pgweb, vault) and OpenSSL-backed
images with Verity's OpenSSL FIPS provider (node, nginx, php, ruby, erlang,
haproxy, httpd, python, postgres, valkey, dotnet).
