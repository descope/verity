...
func (integerMelangeArtifactRunner) Run(_ context.Context, command *melange.Command, _, _ io.Writer) error {
	switch {
	...
	case len(command.Args) > 0 && command.Args[0] == "build":
		...
		writeIntegerMelangeTestFile(command.Dir, filepath.ToSlash(filepath.Join("packages", "repo", arch, "APKINDEX.tar.gz")), "index")
...
func writeIntegerMelangeArtifacts(t *testing.T, options *melange.BuildOptions) error {
	...
	build := *options
	build.Runner = integerMelangeArtifactRunner{}
