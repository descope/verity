...
|---|---|---|---|---|
| **D1** | Pin strategy for initial migration | **Phase 1** (need a copa revision to smoke-test) | **Pin `require` to upstream `main` HEAD, plus a `replace` directive → `verity-org/copacetic feat/go-vcs-resolution` (PR #1546 rebased on upstream main).** The replace is dropped once #1546 merges and we re-pin to a tagged release in Phase 5. Preserves full Go VCS fallback during the migration window. | (A) pin to raw upstream HEAD with `--go-vcs-url` as a no-op — loses Go CVE coverage for 16+ images in `copa-config.yaml` (not acceptable). (B) pin only to tagged releases — forces waiting for both #1546 merge + next tag, likely 2–4 weeks of idle. |
